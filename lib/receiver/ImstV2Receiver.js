...
class ImstV2Receiver extends SerialDevice {
    ...
    dropPartialMessage() {
        if (this.parserBuffer[0] !== END) {
            const markerPos = this.parserBuffer.findIndex(byte => byte === END);
            if (markerPos !== -1) {
                this.parserBuffer = this.parserBuffer.subarray(markerPos + 1);
            }
        }
    }

    checkAndExtractMessage() {
        this.dropPartialMessage();
    ...
    validateResponse(pkg, response) {
        ...
        if (mPkg.setupResponse().messageId != mResponse.messageId) {
            throw new Error(`MessageId mismatch! actual: ${mResponse.messageId} - expected ${mPkg.setupResponse().messageId}`);
        }
    }
