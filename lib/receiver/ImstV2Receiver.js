...
class ImstV2Receiver extends SerialDevice {
    ...
    /**
     * Reject a frame that does not decode or whose CRC is wrong before it is
     * dispatched: it must not be handed to a pending readResponse() as if it
     * were the answer to a command, and it must not reach the telegram parser
     * either - a corrupt telegram would count towards the automatic block list
     * of a device that is working perfectly well.
     */
    isValidMessage(messageBuffer) {
        try {
