...
class ImstV2DeviceMock extends DeviceMock {
    constructor(options) {
        ...
        this.noise = Buffer.from(
            '1234567890abcdefc0c00920d2029649000002c92c446532821851582c067ae1000000046d1906d9180c1334120000426cbf1c4c1300000000326cffff01fd7300d94cc0',
            'hex',
        );
    ...
    getResponse(data) {
        ...
        if (m.destinationId == 0x01 && m.messageId === 0x01) {
            return this.addNoise(m.setupResponse().setPayload(Buffer.alloc(1)).build());
        } else if (m.destinationId == 0x01 && m.messageId === 0x05) {
            return this.addNoise(m.setupResponse().setPayload(this.buildGetFwResponsePayload()).build());
        } else if (m.destinationId == 0x09 && m.messageId === 0x01) {
            return this.addNoise(
                m.setupResponse().setPayload(this.buildGetActiveConfigurationResponsePayload()).build(),
            );
        }
