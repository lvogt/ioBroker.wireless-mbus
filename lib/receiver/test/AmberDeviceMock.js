...
class AmberDeviceMock extends DeviceMock {
    ...
    getResponse(data) {
        ...
        switch (m.commandId) {
            case 0x04:
                return m.setupResponse().setPayload(m.commandId, Buffer.alloc(1)).build();
            case 0x05:
                return m.setupResponse().setPayload(m.commandId, Buffer.alloc(1)).build();
            case 0x09:
                return m.setupResponse().setPayload(m.commandId, this.getSetReqResponse(m)).build();
            case 0x0a:
                return m.setupResponse().setPayload(m.commandId, this.getGetReqResponse(m)).build();
            case 0x0c:
                return m
                    .setupResponse()
                    .setPayload(m.commandId, Buffer.from([0x01, 0x02, 0x03]))
                    .build();
            default:
                throw new Error(`Unimplemented commandId! ${m.commandId.toString(16)}`);
        }
    }
    ...
    getGetReqResponse(m) {
        ...
        switch (address) {
            case 0x05:
                buf[2] = this.cmdOutEnabled ? 0x01 : 0x00;
                break;
            case 0x45:
                buf[2] = this.rssiEnabled ? 0x01 : 0x00;
                break;
            case 0x3f:
                buf[2] = this.autoSleep;
                break;
            default:
                throw new Error(`GET_REQ for address ${address} is unimplemented!`);
        }
        return buf;
    }
    ...
    sendTelegram(dataString, rssi) {
        ...
        const telegram = new AmberMessage().setPayload(3, data).build();

        this.sendData(telegram);
    }
}

exports.SerialPort = AmberDeviceMock;
