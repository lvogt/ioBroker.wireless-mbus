...
class HciMessage {
    ...
    calcMessageSize(): number {
        return (
            4 +
            this.payload.length +
            (this.hasTimestamp ? TIMESTAMP_SIZE : 0) +
            (this.hasRssi ? RSSI_SIZE : 0) +
            (this.hasCrc ? CRC_SIZE : 0)
        );
    }
    ...
    buildHeader(): Buffer {
        ...
        const controlField =
            ...
            (this.hasRssi ? RSSI_FLAG : 0x00) |
            (this.hasTimestamp ? TIMESTAMP_FLAG : 0x00) |
            this.endpointId;
    ...
    build(): Buffer {
        ...
        if (this.hasTimestamp) {
            message.writeUInt32LE(this.timestamp ?? 0, messagePos);
            messagePos += TIMESTAMP_SIZE;
        }

        if (this.hasRssi) {
            const rawRssi = ((((this.rssi ?? 0) + 380.0 / 3.0) * 15.0) / 8.0) & 0xff;
