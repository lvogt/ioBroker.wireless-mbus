...
export type TimerHandle = NodeJS.Timeout | ioBroker.Timeout | undefined;
...
export interface ResponseReader {
    /** whether this message is the answer the command is waiting for */
    accepts: (response: Buffer) => boolean;
    /** hands the answer to the command */
    deliver: (response: Buffer) => void;
}

/** A telegram as handed to the adapter. */
export interface ReceivedTelegram {
    frameType: string;
    /**
     * undefined rather than false where the receiver cannot know: that tells
     * the parser to look for the CRCs instead of promising there are none.
     */
    containsCrc?: boolean;
    rawData: Buffer;
    rssi: number;
...
export interface SerialDeviceOptions {
    ...
    host?: string;
    port?: number | string;
    /** the unit tests emulate a device with a SerialPortStream subclass */
    serialPortImpl?: typeof SerialPort;
    /** the adapter passes its own timers so js-controller can clear them */
    timers?: Partial<TimerFunctions>;
}

export type MessageCallback = (message: ReceivedTelegram) => void;
...
class SerialDevice {
    ...
    port: SerialPort | net.Socket | null;
    mode: string;
    closeRequested: boolean;
    parserBuffer: Buffer;
    maxParserBufferLength: number;
    readTimeout: number;
    readPromises: ResponseReader[];
        ...
        this.timers = {
            ...
            clearTimeout:
                typeof timers?.clearTimeout === 'function'
                    ? timers.clearTimeout
                    : (handle: TimerHandle) => globalThis.clearTimeout(handle as NodeJS.Timeout),
        };
    ...
    buildPayloadPackage(_command: unknown, _payload?: unknown): Buffer | string {
        throw new Error('buildPayloadPackage is unimplemented!');
    }

    validateResponse(_pkg: Buffer | string, _response: Buffer): void {}

    checkAndExtractMessage(): Buffer | null {
        throw new Error('checkAndExtractMessage is unimplemented!');
    }

    parseRawMessage(_messageBuffer: Buffer): ReceivedTelegram {
     ...
     */
    async readResponse(isExpectedResponse?: (response: Buffer) => boolean): Promise<Buffer> {
        let reader: ResponseReader;
        const waitForReadPromise = new Promise<Buffer>(resolve => {
            reader = {
        ...
        try {
            return await Promise.race<Buffer>([waitForReadPromise, timeoutPromise]);
        } finally {
    ...
    async sendPackage(
        command: unknown,
        payload?: unknown,
        isExpectedResponse?: (response: Buffer) => boolean,
    ): Promise<Buffer> {
        ...
        await new Promise((resolve, reject) => {
            // SerialPort and net.Socket both have write(), with different overloads
            (port as net.Socket).write(pkg, (error?: Error | null) =>
                error ? reject(new Error('Error writing to serial connection')) : resolve(true),
            );
        });
    ...
    dispatchMessages() {
        ...
        while (messageBuffer !== null) {
            if (this.isTelegramMessage(messageBuffer)) {
                ...
                this.readPromises.shift()?.deliver(messageBuffer);
            } else {
                // The answer to a command that has given up waiting, a
                // notification the adapter does not use, or a line the waiting
                // command turned down: handing it to the telegram parser
                // produces nonsense at best. Logged rather than passed over in
                // silence, because it is also the only trace left if a
                // receiver ever reports telegrams in a way that
                // isTelegramMessage() does not recognise.
                const hex = Buffer.isBuffer(messageBuffer) ? messageBuffer.toString('hex') : messageBuffer;
                this.log.debug(
                    this.readPromises.length
                        ? `This message answers no pending command - dropped: ${hex}`
                        : `Nothing is waiting for this message - dropped: ${hex}`,
                );
            }
    ...
    handleTcpClose(hadError) {
        if (hadError) {
            ...
            // here as well would report every attempt of an outage that lasts.
    ...
    async init() {
        ...
        try {
            ...
            // Only a trace: the error is thrown on to the adapter, which
            // reports it with the port it belongs to and demotes the attempts
            // that follow while the receiver stays away.
