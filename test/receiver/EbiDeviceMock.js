...
class EbiDeviceMock extends DeviceMock {
    ...
    getResponse(data) {
        ...
        switch (m.messageId) {
            case 0x01:
                return m
                    .setupResponse()
                    .setPayload(m.messageId, Buffer.from([0x40, 0x49]))
                    .build();
            case 0x05:
                setTimeout(this.sendDeviceState.bind(this), 200);
                return m
                    .setupResponse()
                    .setPayload(m.messageId, Buffer.from([0x00]))
                    .build();
            case 0x08:
                return m
                    .setupResponse()
            ...
            case 0x31:
                return m
                    ...
                    .build();
            default:
                throw new Error(`Unimplemented commandId! ${m.messageId.toString(16)}`);
        }
    }
    ...
    sendTelegram(dataString, rssi, frameType, ts) {
        ...
        const telegram = new EbiMessage().setPayload(0xe0, data).build();

        this.sendData(telegram);
    }
}

exports.SerialPort = EbiDeviceMock;
